...
Ideal para administradores de sistemas que utilizan instancias en la nube (GCP, AWS, Azure) y desean evitar sorpresas en su facturación mensual controlando el *Egress traffic*.
...
1. **Clona el repositorio:**
   git clone [(https://github.com/LadyKernel/MyIronGuard/tree/main/network-monitor)]((https://github.com/LadyKernel/MyIronGuard/tree/main/network-monitor))
   cd MyIronGuard/network-monitor
...
Revisa los valores en el archivo .env.

Asegúrate de que los límites (LIMITE_DIARIO o LIMITE_MENSUAL) sean inferiores al tráfico actual para forzar la prueba.

Verifica que tu interfaz de red en el .env coincida con la de tu sistema (puedes verla con ip a).
